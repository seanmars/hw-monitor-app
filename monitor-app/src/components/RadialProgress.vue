...
const finishedPercentage = computed(() => {
  let completed = toNumber(props.completedSteps);
  let total = toNumber(props.totalSteps);
  let target = completed > total ? total : completed;
  return stepSize.value * target;
});
...
const containerStyle = computed(() => ({
  'min-height': `${props.diameter}px`,
  'min-width': `${props.diameter}px`,
}));
...
<template>
  <div class="vrp__wrapper" :style="containerStyle">
    ...
    <svg preserveAspectRatio="xMidYMid meet" :viewBox="`0 0 ${diameter} ${diameter}`" fill="none"
